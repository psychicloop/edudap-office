...
def dashboard():
    ...
    if search_query:
        results['files'] = Quotation.query.filter(
            Quotation.filename.ilike(f'%{search_query}%')
        ).limit(5).all()

        products = ProductData.query.filter(
            or_(
                ProductData.cat_no.ilike(f'%{search_query}%'),
...
def demote_user(user_id):
    user = User.query.get_or_404(user_id)
    # prevent self-demotion and keep at least one admin if you want:
...
@admin_bp.route('/expenses/export', methods=['GET'])
@login_required
def export_expenses():
    si = StringIO()
