...
def upload():
    ...
    try:
        ...
        if ext in ('xls','xlsx','csv'):
            df = pd.read_excel(full_path) if ext!='csv' else pd.read_csv(full_path)
...
'.join([p.extract_text() or '' for p in reader.pages])
    ...
    q = Quotation(uploaded_by=current_user.id, title=title, notes=notes, file_path=path, mime_type=mime, parsed_text=parsed_text,
                  brand=brand, make=make, cas_no=cas_no, product_name=product_name, instrument=instrument, chemical=chemical,
                  reagent=reagent, kit=kit, media=media)
    db.session.add(q)
    db.session.commit()
    upsert_fts(q)
